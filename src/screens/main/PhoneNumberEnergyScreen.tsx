...
type Props = NativeStackScreenProps<RootStackParamList, 'PhoneNumberEnergy'>;

const PhoneEnergyPaywallOverlay = ({
    onBack,
    onUnlock,
}: {
    onBack: () => void;
                ...
                <View style={styles.paywallWrap}>
                    ...
                    <View style={styles.paywallContent}>
                        ...
                        <MysticalText variant="h2" style={styles.paywallTitle}>
                            {t('phoneEnergyPaywallTitle')}
                        </MysticalText>
                        <MysticalText style={styles.paywallSubtitle}>
                            {t('phoneEnergyPaywallSubtitle')}
                        </MysticalText>
...
export const PhoneNumberEnergyScreen: React.FC<Props> = ({ navigation }) => {
    const { t } = useSettings();
    ...
    const canAnalyze = digitsOnly.length > 0;

    const handleAnalyze = () => {
        if (!canAnalyze || isCalculating) return;
        setIsCalculating(true);
        setTimeout(() => {
            const energy = NumerologyEngine.getPhoneNumberEnergy(input);
            setResult(energy);
    ...
    if (!isPro) {
        return (
            <PhoneEnergyPaywallOverlay onBack={handleBack} onUnlock={handleUnlock} />
        );
    }
                    ...
                    >
                            ...
                            <GlassCard style={styles.inputCard} border>
                                <TextInput
                                    ref={inputRef}
                                    style={styles.input}
                                    placeholder={t('phoneEnergyPlaceholder')}
